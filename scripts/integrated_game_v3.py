...
try:
    ...
    print("Warning: stable_baselines3 not found. Using simplified version.")
    PPO = None
...
try:
    ...
    # Simplified Monte Carlo Simulator
    class MonteCarloSimulator:
        def __init__(self, n_simulations=100):
            self.n_simulations = n_simulations
        
        def analyze_decision(self, decision: Decision, scenario: dict) -> dict:
            """Simplified analysis - returns random expected values"""
            # In real implementation, this would run Monte Carlo simulations
            success_rate_range = decision.uncertain_parameters.get('success_rate', (0.5, 0.5))
            mean_success = np.mean(success_rate_range)
            
            base_reward = scenario.get('reward', 10)
            expected_value = (base_reward * mean_success) - decision.cost
            
            return {
                'mean': expected_value,
                'std': expected_value * 0.2,
                'percentile_10': expected_value * 0.7,
                'percentile_90': expected_value * 1.3
            }
...
class EnhancedStrategicAgent:
    ...
    def choose_strategy(self, env, other_agents=None):
        ...
        if self.opponent_model and other_agents:
            ...
            for agent in other_agents:
                if agent.agent_id != self.agent_id:
                    analysis = self.opponent_model.get_analysis(opp_idx)
    ...
    def update_opponent_models(self, other_agents):
        ...
        for agent in other_agents:
            if agent.agent_id != self.agent_id:
                ...
                self.opponent_model.update_belief(opp_idx, obs)
                opp_idx += 1
    ...
    def draw_info_panel(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if agent.opponent_model and len(self.agents) > 1:
                ...
                opp_idx = 0
                for opp_agent in self.agents:
                    if opp_agent.agent_id != agent.agent_id:
                        analysis = agent.opponent_model.get_analysis(opp_idx)
                        
                        # Show dominant belief
                        belief_text = self.font_small.render(
                            f"→ A{opp_agent.agent_id}: {analysis['dominant_strategy'][:3]} "
